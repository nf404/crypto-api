;(
    ...
    function (CryptoApi) {
        ...
        Hmac.prototype.constructor = function(key, hasher, options){
            ...
            if (this.key.length < blockSizeInBytes) {
                this.key.concat(Array.apply(null, Array(blockSizeInBytes - this.key.length)).map(Number.prototype.valueOf, 0x00));
            }
            this.iKeyPad = Array.apply(null, Array(blockSizeInBytes)).map(Number.prototype.valueOf, 0x36);
            this.oKeyPad = Array.apply(null, Array(blockSizeInBytes)).map(Number.prototype.valueOf, 0x5c);
