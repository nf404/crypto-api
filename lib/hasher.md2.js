(
    function (root, factory) {
        "use strict";
        if (typeof root.CryptoApi === 'undefined') {
            root.CryptoApi = require('./crypto-api');
        }
        return factory(root.CryptoApi);
    }(this, /**
     * @param {CryptoApi} CryptoApi
     * @returns {Md2}
     */
    function (CryptoApi) {
        'use strict';

        /**
         * @class Md2
         * @extends BaseHasher
         * @implements HasherInterface
         * @classdesc Md2 hasher
         */
        var Md2 = function () {
            this.constructor();
        };
        Md2.prototype = Object.create(CryptoApi.BaseHasher.prototype);
        /**
         * @memberOf Md2
         * @constructor
         */
        Md2.prototype.constructor = function(){
            CryptoApi.BaseHasher.prototype.constructor.call(this);
            /**
             * @desc Hash state
             * @memberOf! Md2#
             * @alias state.hash
             * @type {number[]}
             */
            this.state.hash = new Array(48);
            /**
             * @desc Checksum
             * @memberOf! Md2#
             * @alias state.checksum
             * @type {number[]}
             */
            this.state.checksum = new Array(16);
        };

        /**
         * @desc Constants from Pi
         * @link https://github.com/e-sushi/MD2-S-box-creator
         * @memberOf Md2#
         * @member {number[]} piSubst
         */
        Md2.prototype.piSubst = [
            ];
        /**
         * @desc Size of unit = 1 byte
         * @memberOf Md2#
         * @member {number} unitSize
         */
        Md2.prototype.unitSize = 1;

        /**
         * @memberOf Md2
         * @method processBlock
         * @param {number[]} block
         */
        Md2.prototype.processBlock = function processBlock(block) {
            // Append hash
            for (var i = 0; i < 16; i++) {
                this.state.hash[16 + i] = block[i];
                this.state.hash[32 + i] = this.state.hash[16 + i] ^ this.state.hash[i];
            }

            // 18 Rounds
            var t = 0;
            for (i = 0; i < 18; i++) {
                for (var j = 0; j < 48; j++) {
                    t = this.state.hash[j] ^= this.piSubst[t];
                }
                t = (t + i) & 0xff;
            }

            // Append checksum
            t = this.state.checksum[15];
            for (i = 0; i < 16; i++) {
                t = this.state.checksum[i] ^= this.piSubst[block[i] ^ t];
            }
        };

        /**
         * @memberOf Md2
         * @method finalize
         * @return {HashArray}
         */
        Md2.prototype.finalize = function finalize() {
            // Add padding
            var padding = this.message.length & 0xf;
            this.update(new Array(17 - padding).join(String.fromCharCode(16 - padding)));

            // Process checksum
            this.update(this.state.checksum);

            // Return hash
            return CryptoApi.hashArray(this.state.hash.slice(0, 16));
        };

        CryptoApi.Hashers.add('md2', Md2);
        return Md2;
    })
);